...
if [ -f $EOS_BINARY_DIR/Tests/C++/NakVapor.val ] ; then exit 0; fi

if [ ! -d $NAK_ROOT_DIR && ! -f $NAK_ROOT_DIR ] ; then
    error 44 " T.M. Plugin Nak not found, --with-nak=$NAK_ROOT_DIR"
fi

# Decompression if plugin is an archive
if [ -f $NAK_ROOT_DIR ]; then
    mkdir -p $ROOT_BINARY_DIR/plugins_tmp
    tar xf $NAK_ROOT_DIR -C $ROOT_BINARY_DIR/plugins_tmp
    NAK_ROOT_DIR=$ROOT_BINARY_DIR/plugins_tmp/Nak
fi
...
MV_FILE=$NAK_ROOT_DIR/model-version.txt
...
PLUGINEXT_TCS=$NAK_ROOT_DIR/TCS
...
:> $INDEX_TMFILE
#
# Copy file from $NAK_ROOT_DIR to $BINARY_DIR
echo "Copy plugin EOS_Nak source files in EOS/Src"
for file in `find $NAK_ROOT_DIR -name "*.?xx"` ; do
    if [ -f $file ] ; then
...
if [ -f $INDEX_TMFILE ] ; then
   echo "Nak  NakLiquid  EOS_Nak  Nak  Unknown  1 Liquid" >> $INDEX_TMFILE
   echo "Nak  NakVapor   EOS_Nak  Nak  Unknown  1 Vapor" >> $INDEX_TMFILE
fi
...
if [ -d $PLUGINEXT_TCS ] ; then
   ...
   echo "WARNING : You don't have physics tests for Nak model"
